...
CapturePin::CapturePin(CaptureFilter* filterIn, IDeviceSource* sourceIn, const GUID& expectedMajorType, const GUID& expectedMediaType)
...
{
    *pPinDir = PINDIR_INPUT; return NOERROR;
}
STDMETHODIMP CapturePin::QueryId(LPWSTR* lpId)
{
    *lpId = const_cast<LPWSTR>(L"Capture Pin"); return S_OK;
}
...
CaptureFilter::CaptureFilter(IDeviceSource* source, const GUID& expectedMajorType, const GUID& expectedMediaType)
...
{
    *pClock = NULL; return NOERROR;
}
STDMETHODIMP CaptureFilter::Stop()
{
    pin->EndFlush(); state = State_Stopped; return S_OK;
}
...
{
    return (++curPin > 1) ? S_FALSE : S_OK;
}
STDMETHODIMP CaptureEnumPins::Reset()
{
    curPin = 0; return S_OK;
}
